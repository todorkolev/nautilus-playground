...
class MeanReversionStrategy(Strategy):
    ...
    def rebalance_portfolio(self) -> None:
        ...
        self._log.info(f"Instrument: {self.instrument}")
        self._log.info(f"Lookback period: {self.lookback}")
        self._log.info(f"Prices array length: {len(prices)}")
        self._log.info(f"Returns array length: {len(returns)}")
